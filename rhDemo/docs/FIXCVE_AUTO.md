...
| **Versions corrigées Trivy : liste complète + garde-fou d'appartenance + repli par candidate** | `fixcve-detect.py` conserve **toutes** les versions listées par le champ `FixedVersion` de Trivy dans `fixed_versions` (`fixed_version_hint` n'en reste que le 1er élément, gardé pour compat). `fixcve-auto-lookup/SKILL.md` étape 2 : quand `fixed_versions` est non vide, la `target_version` doit en être l'une — `fixcve-validate-json.py` (schéma lookup) **rejette** toute `target_version` hors de cette liste, sauf pour un paquet OS `ecosystem: "docker"` pur (où `fixed_versions` est une version de paquet distro, sans rapport avec le tag d'image). Les candidates (entrées strictement supérieures à `installed_version`) sont ordonnées croissant, même branche mineure d'abord ; un `fixed_version_hint`/`fixed_versions` uniquement inférieur ou égal à `installed_version` reste rejeté (rétrograde — branches de maintenance parallèles, ex. Keycloak 26.4.x/26.6.x). Pour un cas « image externe » (jar Trivy dont `image` ≠ `rhdemo-app`), les candidates sont **essayées dans l'ordre** et la 1re dont `docker manifest inspect <image>:<candidate>` résout un digest est retenue : les numéros `FixedVersion` sont des versions de module/jar qui ne correspondent pas toujours à un tag d'image publié. `no_fix_available` seulement si **aucune** candidate ne résout. Incidents : build #812 (`26.4.15` rétrograde), #818/#819 (`26.6.3` retenue par recherche registre alors que Trivy listait `26.4.15, 26.6.6, 26.7.2` — `26.6.3` non corrigée pour CVE-2026-18963, d'où re-détection CRITICAL et rollback), puis #823 (`26.6.6` bien lue et même branche, mais **inexistante en tag d'image** — `quay.io/keycloak/keycloak` s'arrête à `26.6.4` sur la branche `26.6` — l'ancienne règle concluait `no_fix_available` au lieu d'itérer vers `26.7.2`, tag réel et corrigé). |
