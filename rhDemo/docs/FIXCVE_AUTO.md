...
| **Validation du `fixed_version_hint` Trivy avant de le retenir** | `fixcve-auto-lookup/SKILL.md` étape 2 : un `fixed_version_hint` inférieur ou égal à `installed_version` est rejeté plutôt que reprise telle quelle — plusieurs branches de maintenance en parallèle (ex. Keycloak 26.4.x/26.6.x) peuvent faire pointer Trivy vers une version corrigée mais **plus ancienne** que celle installée. Incident ayant motivé ce garde-fou : build #812, `keycloak-services` (Trivy indiquait `26.4.15` corrigé alors que `26.6.2` était installé). |
...
- **Remplacer le polling par un `post { failure { ... } }` dans `Jenkinsfile-CI`**, déclenchant un job dédié (`RHDemo-fixcve-auto`) avec build number + type de stage en paramètres. Gain principal : Jenkins sait *nativement* quel stage a échoué, ce qui élimine la fragilité de la détection actuelle par `wfapi/describe` — un échec précoce y fait passer tous les stages en aval en non-SUCCESS, d'où la logique « premier stage non-SUCCESS = cause racine » de `fixcve-auto-poll.sh` (voir le script).
