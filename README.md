...
- Secrets applicatifs : choix d'utiliser le chiffrement des valeurs de clés contenant des secrets avec SOPS et de les commiter dans Git. L'utilisaiton d'un outil centralisé de type Hashicorp Vault demanderait une expertise spécialisée (TODO).
...
- Politique de mots de passe et MFA (TODO en mettant en oeuvre les fonctionnalités Keycloak).
- Contrôles d’accès : RBAC, les roles des utilisateurs sont portés par Keycloak et transmis à Spring Boot dans  l'idtoken OIDC.

## Installation 
- Tout,  y compris le staging doit tenir sur un PC récent 16Go sous linux (testé sur Ubuntu)
- Git
...
- Utiliser la chaine CI/CD et déployer l'environnement de staging
   ...
   - Lancez le pipeline décrit dans rhDemo/Jenkinsfile
